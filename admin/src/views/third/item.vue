...
<template>
  ...
  >
    <el-form ref="formRef" label-width="auto" :model="formData" :rules="rules" class="colon">
        ...
        <el-input v-model.trim="data.authUrl" clearable>
          <template #suffix>
            <el-button
              ...
              type="primary"
              @click="
                setting(data.authParams || {}, '认证请求参数')?.then(
                  (value) => (data.authParams = value),
                )
              "
            />
          </template>
        ...
        <el-input v-model.trim="data.verifyUrl" clearable>
          ...
          <template #suffix>
            <el-button
              ...
              type="primary"
              @click="
                setting(data.verifyParams, '校验请求参数')?.then(
                  (value) => (data.verifyParams = value),
                )
              "
            />
          </template>
        ...
        <el-input v-model.trim="data.userUrl" clearable>
          ...
          <template #suffix>
            <el-button
              ...
              type="primary"
              @click="
                setting(data.userParams, '用户信息请求参数')?.then(
                  (value) => (data.userParams = value),
                )
              "
            />
          </template>
      ...
      <el-alert
        >使用 $auth.xyz 获取认证地址回调参数，$verify.xyz 获取校验接口返回参数，$user.xyz
        获取用户信息接口返回参数</el-alert
      >
    </el-form>
    <KeyMap ref="keyRef" />
    <template #footer>
