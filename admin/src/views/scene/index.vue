<script setup lang="ts">
  ...
  onMounted(() => {
    loading.value = true;
    Promise.all([loadScene(), loadStory(), loadItem(), loadTarget()]).then(() => {
      loading.value = false;
    });
  });
  function loadScene() {
  ...
  function addScene(scene?: Scene) {
    ...
    scenes.value.forEach((s) => {
      if (
        ...
        s.position.x < position.x + 40 &&
        s.position.y >= position.y &&
        s.position.y < position.y + 40
      ) {
        position.x += 40;
        position.y += 40;
      }
  ...
  function gotoPlay() {
    const url = type === 'draft' ? `/d/${storyId}` : `/s/${storyId}`;
    ...
    return scenes.value
      .filter(
        (scene) =>
          ...
          scene.options.some((option) => option.next === connectScene.value) ||
          currentScene?.options.some((option) => option.next === scene.name),
      )
      .map((scene) => ({
        name: scene.name,
        conns: scene.options
          .filter(
            (option) =>
              ...
              (scene.name === connectScene.value || option.next === connectScene.value),
          )
          .map((option) => ({
      ...
      >
        <el-container class="pt-60px">
            ...
            <el-form class="!space-x-2" :size="isMobile ? 'small' : 'default'">
              <el-button-group>
                ...
                <ButtonEx
                  ...
                  content="跳转到起始场景"
                  type="primary"
                  @click="highlightScene(story.start)"
                  plain
                />
                <ButtonEx
                  icon="i-mdi:movie-open-plus-outline"
                  type="primary"
                  @click="addScene()"
                  plain
                >
                ...
                <ButtonEx
                  :icon="!isVirtual ? 'i-solar:play-bold' : 'i-solar:stop-bold'"
                  type="success"
                  @click="virtualRun"
                  :plain="!isVirtual"
                >
          ...
          <el-main class="!h-full !overflow-unset">
            <section
              v-show="viewMode.includes('panel')"
            ...
            >
              ...
              >
                ...
                <SceneItem
                  ...
                  @next="highlightScene"
                  @edit="editScene"
                  @connect="(connectScene = $event.name), (focusTag = '')"
                  @focus-tag="focusTag = focusTag != $event ? $event : ''"
                  :focus-tag="focusTag"
                  @move-start="moveSync"
                  @moving="ConnectRef?.render()"
                  @remove="removeScene"
                  @start="setStart"
                  @copy="copyScene"
                  @mousedown.stop
                  ...
                  :class="{
                    ...
                    'opacity-50': connectScene && !connections[scene.name],
                  }"
            ...
            <section
              v-if="viewMode.includes('network')"
              @mousedown.stop
              class="fixed top-0 left-0 right-0 bottom-0 w-full h-full z-3 flex-1 bg-black bg-opacity-50"
            >
              <SceneMap :scenes="scenes" @node-click="highlightScene($event.name)" />
            </section>
            <section
