<script setup lang="ts">
  ...
  async function save() {
    ...
    if (
      props.options.some(
        (opt) => opt.text !== data.value.text && opt.id && opt.id === data.value.id,
      )
    ) {
    ...
    <el-form ref="formRef" :model="data" label-width="auto" :rules="rules">
      <el-form-item label="唯一标识" prop="id">
        <el-input
          v-model="data.id"
          clearable
          placeholder="给选项按钮添加 id 属性，可在 CSS 中使用 option_xxx 添加样式"
        />
      </el-form-item>
        ...
        <template #label>
          <span>
            <el-tooltip placement="top">
              <template #content>
                <p
                  >追加到场景内容的文本。在场景内容，可以通过<code>${选项}</code>来控制追加内容的位置。当选项存在时插入。</p
                >
              </template>
