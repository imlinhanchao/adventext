...
<template>
  <el-header class="!bg-transparent !p-3 !pb-1 relative" height="auto">
    <Menu
      v-if="screenSM && getUserInfo.isAdmin"
      @menu-click="$router.push($event)"
      class="z-1000"
    />
    <span :class="{ 'absolute left-0 right-0 flex justify-center z-0': screenSM }"
      ><BreadCrumb
    /></span>
    <Logo v-if="!screenSM" />
